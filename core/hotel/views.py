...
class PayoutCreateView(APIView):
    """
    Create a payout link for partners to receive their earnings
    
    POST: Generate a Stripe payout link
          - Requires: amount
          - Partner must have approved hotel
          - Creates Payout record with shareable URL
    """
    permission_classes = [IsAuthenticated]
    
    @extend_schema(
        request={'application/json': {'type': 'object', 'properties': {'amount': {'type': 'number'}}}},
        responses={200: {'type': 'object', 'properties': {'payout_url': {'type': 'string'}}}},
        tags=['Payouts'],
        description="Create payout link for partner earnings"
    )
    def post(self, request):
        try:
            user = request.user
            ...
            # Check if partner has approved hotel
            try:
                hotel = Hotel.objects.get(partner=user)
                if hotel.is_approved != 'approved':
                    return Response({
                        'error': 'Only partners with approved hotels can request payouts'
                    }, status=status.HTTP_403_FORBIDDEN)
            except Hotel.DoesNotExist:
                return Response({
                    'error': 'Partner must have a hotel to request payout'
                }, status=status.HTTP_403_FORBIDDEN)
            
            # Generate Stripe payout URL
            # For now, create a simple payout link (in production, use Stripe API)
            import uuid
            ...
            payout = Payout.objects.create(
                ...
                amount=amount,
                status='pending',
                description=f"Payout request for {hotel.hotel_name}"
            )
            
            # Generate payout URL (can be customized)
            payout_token = str(uuid.uuid4())
            payout_url = request.build_absolute_uri(
                f'/api/hotel/payouts/{payout.id}/complete/?token={payout_token}'
            )
            
            payout.payout_url = payout_url
            payout.save()
...
class PayoutCompleteView(APIView):
    """
    Complete payout for a partner
    
    GET: View payout details and complete payout
         Partners use the link generated by PayoutCreateView
    """
    permission_classes = [AllowAny]  # Payout link is token-protected
    ...
    def get(self, request, pk):
        try:
            ...
            # Verify token (basic protection)
            if not token:
                return Response({
                    'error': 'Invalid payout link'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({
                'error': f'Payout completion failed: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


class PayoutListView(APIView):
    """
    List all payouts for authenticated partner
    
    GET: List partner's payouts with pagination
    """
    permission_classes = [IsAuthenticated]
    
    @extend_schema(
        tags=['Payouts'],
        description="List partner's payouts"
    )
    def get(self, request):
        try:
            from .models import Payout
            ...
            user = request.user
            payouts = Payout.objects.filter(partner=user).order_by('-created_at')
            ...
            payouts_data = []
            for payout in payouts[start:end]:
                payouts_data.append({
                    'id': payout.id,
                    'amount': str(payout.amount),
                    'currency': payout.currency,
                    'status': payout.status,
                    'hotel': payout.hotel.hotel_name if payout.hotel else 'N/A',
                    'created_at': payout.created_at.isoformat(),
            ...
            return Response({
                'error': f'Failed to fetch payouts: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
